...
function listen(api_key, controller) {
  controller.hears(['(video|youtube|vid|vj)( me)?( of|:)? (.*)'],
  'direct_message,direct_mention,mention',
    function(bot, message) {
      var query = message.match[4];
      search(api_key, query, get_success_handler(bot, message, query), get_error_handler(bot, message));
    }
  );
}
...
function get_error_handler(bot, message) {
  return function() {
